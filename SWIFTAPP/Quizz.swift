...
//private func getRandomQuizItems(from items: [QuizItem], count: Int = 5) -> [QuizItem] {
...
//Resultats
    ...
    var body: some View {
                    ...
                    // Gifts + score + bear
                                      ...
                                      HStack(spacing: 22) {
                                          ...
                                          if currentIndex < max(state.questions.count - 1, 0) {
                                              CircleButton(icon: "chevron.right", tint: accent) {
                                                  goNext()
                                              }
                                          } else {
                                              // Finish
                                              CircleButton(icon: "checkmark", tint: accent) {
                                                  finish()
                                              }
        ...
        // 👉 Ouvre l’écran de résultats et déclenche onFinish au retour
    ...
    // MARK: - Logic
        private func setup() {
            let items = loadQuizItems(from: quizFile, exactly: 5)
        ...
        private func finish() {
            // Passe seulement si tout est vert (5/5)
            let correct = correctCount
            passedResult = (correct == state.questions.count && state.questions.count == 5)
            showResults = true
        }
    }
